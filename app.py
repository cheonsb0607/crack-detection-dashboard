...
>>> import streamlit as st import numpy as np import matplotlib.pyplot as plt import seaborn as sns import os from PIL import Image
... 
...
... 
... if page == "1. 이미지 업로드": st.header("📷 이미지 업로드") method = st.radio("이미지 업로드 방법 선택", ["파일 업로드", "카메라 촬영"])
... 
... if method == "파일 업로드":
...
... 
... elif page == "2. 모델 성능": st.header("📊 모델 성능 평가") tp = st.number_input("TP (True Positive)", min_value=0, value=50) tn = st.number_input("TN (True Negative)", min_value=0, value=40) fp = st.number_input("FP (False Positive)", min_value=0, value=5) fn = st.number_input("FN (False Negative)", min_value=0, value=5)
... 
... total = tp + tn + fp + fn
...
elif page == "3. 예측 결과": st.header("🧠 예측 결과 및 위험도 평가") uploaded_images = st.session_state.get('uploaded_images', None)
...
if not uploaded_images:
    ...
    for idx, file in enumerate(uploaded_images):
        ...
        with col2:
            # 아래는 예시용 위험도 판단 (실제 모델 출력값으로 교체 필요)
            dummy_loss = np.random.rand()
            if dummy_loss < 0.2:
                level = "🟢 안전"
            elif dummy_loss < 0.5:
                level = "🟡 주의"
